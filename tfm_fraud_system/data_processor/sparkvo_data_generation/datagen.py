...
def generate_data(**kwargs):
    print(kwargs)
    num_cust = kwargs.get('nb_customers')
    seed_num = kwargs.get('seed')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-config', type=pathlib.Path, nargs='?', help='Profile config file (typically profiles/main_config.json")', default='/app/tfm_fraud_system/data_processor/sparkvo_data_generation/profiles/main_config.json')
    ...
    for profile_file in configs.keys():
        ...
        while customer_file_offset_start <= max(num_cust - 1, chunk_size):
            ...
            args_array.append((
                customers_out_file,
                pathlib.Path(os.path.join('/app/tfm_fraud_system/data_processor/sparkvo_data_generation/profiles', profile_file)),
                start_date, 
                end_date, 
                transactions_filename,
                customer_file_offset_start,
